...
class UdemyTestEmitter
{
    ...
    private static function styleHelperLines()
    {
        return array(
            ...
            '}',
            'function wgCssNumericEquals(actual, expected, epsilon) {',
            ...
            '}',
            'function wgParseDomMatrix(transform) {',
            '    if (!transform || transform === "none") return null;',
            '    if (typeof DOMMatrix !== "function") return null;',
            '    try { return new DOMMatrix(transform); } catch (e) { return null; }',
            '}',
            'function wgFindTransform(element) {',
            '    var el = element;',
            '    while (el && el.nodeType === 1) {',
            '        var style = window.getComputedStyle(el);',
            '        var transform = style && style.transform;',
            '        if (transform && transform !== "none") {',
            '            return { element: el, transform: transform, matrix: wgParseDomMatrix(transform) };',
            '        }',
            '        var zoom = style && style.zoom;',
            '        if (zoom && zoom !== "normal") {',
            '            var zNum = parseFloat(zoom);',
            '            if (isFinite(zNum) && Math.abs(zNum - 1) > 0.001) {',
            '                return { element: el, transform: "zoom(" + zoom + ")", matrix: null, zoom: zoom };',
            '            }',
            '        }',
            '        el = el.parentElement;',
            '    }',
            '    return null;',
            '}',
            'function wgResolveValue(el, prop, value) {',
            '    var raw = String(value == null ? "" : value).trim();',
            '    if (!raw || !document.createElement || !window.getComputedStyle) return raw;',
            '    var parent = el && el.nodeType === 1 ? el : document.body;',
            '    if (!parent) return raw;',
            '    var probe = document.createElement("wg-probe");',
            ...
            '    }',
            '    if (wgIsOffsetProperty(prop)) {',
            '        probe.style.setProperty("position", "fixed", "important");',
            '    }',
            '    parent.appendChild(probe);',
            '    var resolved = window.getComputedStyle(probe).getPropertyValue(prop).trim();',
            '    parent.removeChild(probe);',
            '    return resolved || raw;',
            '}',
            'function wgComputedEqual(prop, actual, expected, el) {',
            '    actual = String(actual || "").trim();',
            '    expected = String(expected || "").trim();',
            '    if (wgIsColorProperty(prop)) {',
            '        return wgNormalizeColor(actual) === wgNormalizeColor(expected);',
            '    }',
            ...
            '        var am = wgParseDomMatrix(actual);',
            '        var em = wgParseDomMatrix(expected);',
            '        if (!am || !em) return actual === expected;',
            ...
            '    }',
            '    if (wgIsOffsetProperty(prop)) {',
            '        actual = wgNormalizeOffset(actual);',
            '        expected = wgNormalizeOffset(expected);',
            '    }',
            '    var resolved = wgResolveValue(el, prop, expected);',
            '    if (wgIsOffsetProperty(prop)) resolved = wgNormalizeOffset(resolved);',
            '    var numeric = wgCssNumericEquals(actual, resolved, 0.01);',
        ...
        switch ($type) {
            ...
            case 'computed_style_equals':
                ...
                return array(
                    ...
                    'expect(wgComputedEqual(prop, actual, expected, el) ? expected : actual).toBe(expected);',
                );
            ...
            case 'computed_styles_equals':
                ...
                foreach ($test['expected'] as $prop => $expected) {
                    ...
                    $lines[] = '    expect(wgComputedEqual(prop, actual, expected, el)'
